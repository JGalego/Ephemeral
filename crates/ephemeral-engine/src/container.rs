//! Starting, stopping and watching what is already running.
//!
//! Every operation here does the same three things in the same order, and the
//! order is the point: the *intention* is recorded first, then the runtime is
//! asked, then the *fact* is recorded by the runtime as actor. A crash between
//! the second and third leaves an application in `Starting`, which is a state
//! the machine has — rather than a manifest that claims something untrue.
//!
//! Nothing here prints. Each operation returns what happened, with the
//! sentences a person reads already written, so that a window and a terminal
//! report the same run the same way.
...
use ephemeral_runtime::{
    ContainerState, ContainerStatus, ManagedContainer, Runtime as _, Secrets, docker::DockerRuntime,
};
...
#[derive(Debug, Clone)]
pub struct Started {
    /// The container now holding it, when the runtime named one.
    pub container: Option<String>,

    /// What it can reach, one line each, already worded for a person.
    pub confinement: Vec<String>,

    /// Access that was granted and is not being given effect.
    pub refused: Vec<String>,

    /// Capabilities the person allowed that Ephemeral itself may not carry out.
    ///
    /// Said before it starts rather than discovered afterwards: an application
    /// whose grants are inert runs with less than the person allowed it, and
    /// the only other clue is that it can see nothing of theirs — which reads
    /// as a sandbox working rather than a permission missing.
    pub inert: Option<String>,
}

/// What reconciling an application's record against its container did.
#[derive(Debug, Clone)]
pub struct Reconciled {
    /// Where the record was.
    pub was: LifecycleState,

    /// Where it is now. The same as `was` when they already agreed.
    pub now: LifecycleState,

    /// Why it moved, when it moved.
    pub because: Option<String>,

    /// Whether this application is holding a container at all.
    pub holds_a_container: bool,
}
...
/// One thing a sweep did about one application.
#[derive(Debug, Clone)]
pub struct Sweep {
    /// Which application.
    pub app: AppId,
...
///
/// `arguments` are appended to the entrypoint rather than replacing it: an
/// application's entry point is part of what it *is*, recorded in its version,
/// and letting a command line replace it would let somebody run something other
/// than the application they are looking at.
///
/// # Errors
///
/// If the lifecycle does not allow starting, if Ephemeral may not drive a
/// container runtime, if there is none to drive, or if the runtime refuses.
pub fn start(
    ...
    let confined = specification(workspace, manifest)?;
    let mut spec = confined.spec;
    ...
    match runtime.start(&spec, &Secrets::new()) {
        Ok(status) => {
            apply(
                workspace,
                manifest,
            ...
            Ok(Started {
                ...
                confinement: confinement(&spec),
                refused: confined.refused,
                inert: confined.inert,
            })
        }
        Err(error) => {
            apply(
...
///
/// The container is gone once it has stopped: everything the application keeps
/// lives in its data directory, which outlives it. Leaving a stopped container
/// behind would make a list of applications and Docker disagree about what
/// exists.
///
/// # Errors
///
/// If the lifecycle does not allow stopping, or the runtime cannot be reached.
pub fn stop(workspace: &mut Workspace, manifest: &mut AppManifest, reason: &str) -> Result<()> {
...
/// # Errors
///
/// If the lifecycle does not allow pausing, or the runtime cannot be reached.
pub fn pause(workspace: &mut Workspace, manifest: &mut AppManifest, reason: &str) -> Result<()> {
...
/// # Errors
///
/// If the lifecycle does not allow resuming, or the runtime cannot be reached.
pub fn resume(workspace: &mut Workspace, manifest: &mut AppManifest, reason: &str) -> Result<()> {
...
/// # Errors
///
/// If the runtime cannot be reached, or the state machine refuses a step that
/// what the container is doing implies.
pub fn reconcile(workspace: &mut Workspace, manifest: &mut AppManifest) -> Result<Reconciled> {
    let was = manifest.lifecycle.state();

    if !was.requires_runtime() {
        return Ok(Reconciled {
            was,
            now: was,
            because: None,
            holds_a_container: false,
        });
    }
    ...
    let steps = implied_events(was, &observed);
    if steps.is_empty() {
        return Ok(Reconciled {
            was,
            now: was,
            because: None,
            holds_a_container: true,
        });
    }
    ...
    for (event, actor, reason) in steps {
        apply(workspace, manifest, event, actor, &reason)?;
    }
    save(workspace, manifest)?;

    Ok(Reconciled {
        was,
        now: manifest.lifecycle.state(),
...
#[must_use]
pub fn has_output(manifest: &AppManifest) -> bool {
    manifest.runtime.is_some()
}

/// What the application itself has printed, when there is a container to ask.
///
/// Best-effort by design: a missing runtime or a container that has gone is a
/// quiet `None` rather than a failure. A container outlives the run that ended
/// it, so a crashed application still has one — and the last thing it printed
/// is the most useful thing on the machine.
#[must_use]
pub fn output(manifest: &AppManifest, lines: u32) -> Option<String> {
    ...
    runtime.logs(&manifest.id, lines).ok()
}
...
/// once (ADR-0003).
pub fn reaches(subject: &ephemeral_core::Principal, manifest: &AppManifest) -> bool {
...
/// # Errors
///
/// If the applications cannot be listed or a manifest cannot be saved. A
/// runtime that is not answering is not an error here: the revocation stands
/// either way, and refusing it because Docker is down would leave the
/// permission in place, which is the worse of the two failures.
pub fn stop_what_lost_a_permission(
    workspace: &mut Workspace,
...
/// # Errors
///
/// If the applications cannot be listed, the runtime cannot be asked what a
/// container is doing, or a corrected manifest cannot be saved.
pub fn sweep(workspace: &mut Workspace, runtime: &DockerRuntime) -> Result<Vec<Sweep>> {
    ...
    for id in watched {
        ...
        if let Some(reason) = breach {
            ...
            for (event, actor) in [
                ...
                apply(workspace, &mut manifest, event, actor, &reason)?;
            }
            workspace.audit_mut().append(
                Actor::Ephemeral,
                AuditEvent::SandboxDestroyed {
...
/// # Errors
///
/// If the container runtime cannot be asked what it is holding.
pub fn orphans(workspace: &Workspace, runtime: &DockerRuntime) -> Result<Vec<Orphan>> {
    let held = runtime
        .managed_containers()
...
/// Removes leftover containers, and records that it did.
///
/// Removal goes through the same path a stop does, so it cannot reach a
/// container Ephemeral did not name: anything whose name does not carry
/// Ephemeral's own prefix is skipped rather than guessed at. Reaping by
/// anything looser would be an efficient way to destroy somebody's unrelated
/// work.
///
/// # Errors
///
/// If a container cannot be removed, or the record cannot be saved.
...
mod tests {
    use super::*;
    use crate::sandbox::{
        argument_hint, describe_mount, explain_refusal, granted_permissions, specification,
        usable_runtime,
    };
    use ephemeral_core::{
        Principal,
        lifecycle::{TransitionContext, TransitionRequest},
        manifest::{AppInterface, RuntimeKind, RuntimeSpec},
        permission::{AppPermission, PathScope, Permission},
    };
    use ephemeral_runtime::ContainerSpec;
    ...
    fn a_specification_is_built_from_granted_permissions_only() {
        ...
        let spec = specification(&workspace, &built()).unwrap().spec;
